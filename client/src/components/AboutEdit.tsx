...
const About: React.FC<{}> = () => {
  ...
  useEffect(() => {
    const getInitValues = async () => {
      ...
      // const { imgType, img, name } = data.selectedProfileImg;

      // console.log("img.toString", img?.toString());
      // console.log("img data", img);
      // console.log("img type", imgType);
      // console.log("name", name);

      // const test = Buffer.from(img.data).toString("base64");

      // console.log("ASD-123", test);
      ...
      // setSource(`data:${imgType};charset=utf-8;base64,${test}`);
   ...
   * const test = Buffer.from(img.data).toString("base64");
      ...
      setSource(`data:image/jpeg;charset=utf-8;base64,${test}`);
   */
          ...
          <div className="card-columns">
            {hasUploadedImgs &&
              initValues.uploadedImgs.map((itemImg) => {
                const { id, imgType, img, name } = itemImg;
                let dataBase64;
                ...
                return (
                  ...
                  >
                    <img
                      ...
                      alt={name}
                      src={src}
                      // src={config.WEBSITE_API + "/thumbnails/" + itemImg.name}
                    />
                    <div className="form-group">
                      <p>{name}</p>
                      <button
